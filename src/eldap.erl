...
start_link(Name) when is_atom(Name)->
    start_link(atom_to_binary(Name, latin1));

start_link(Name) ->
    Reg_name = erlang:binary_to_atom(<<"eldap_",
        			       Name/binary>>, utf8),
    ?INFO_MSG("Starting ~s...", [Reg_name]),   
...
init([Hosts, Port, Rootdn, Passwd, Opts]) ->
    Encrypt = case eldap_utils:get_opt(encrypt, Opts,
                                   fun(tls) -> tls;
                                      (starttls) -> starttls;
                                      (none) -> none
                                   end) of
    ...
    CacertOpts = case eldap_utils:get_opt(
                        tls_cacertfile, Opts,
    ...
    DepthOpts = case eldap_utils:get_opt(
                       tls_depth, Opts,
    ...
    Verify = eldap_utils:get_opt(tls_verify, Opts,
                             fun(hard) -> hard;
                                (soft) -> soft;
                                (false) -> false
                             end, false),
...
polish([H | T], Res, Ref)
    ...
    F = fun ({_, A, V}) -> {A, V} end,
    ?DEBUG("SearchResultEntry.attributes ~p",[H#'SearchResultEntry'.attributes]),
